 ...
 * @2014年9月11日
 */
public class BuffImageLSource extends LuminanceSource {
	private final BufferedImage image;
	private final int left;
	private final int top;

	public BuffImageLSource(BufferedImage image) {
		this(image, 0, 0, image.getWidth(), image.getHeight());
	}

	public BuffImageLSource(BufferedImage image, int left,
	...
	public LuminanceSource crop(int left, int top, int width, int height) {
		return new BuffImageLSource(image, this.left + left,
	...
	public LuminanceSource rotateCounterClockwise() {
		...
		g.dispose();
		int width = getWidth();
		return new BuffImageLSource(rotatedImage, top,
